...
console.log(user , "USER FROM CONTAT PAGE");
  ...
  return (
   <div className='w-full grid place-items-center'>
    {
        contactLandlord ? <div className='w-[52%] mx-auto '>
        <h1>Contact <strong>{user?.username} </strong> for <strong>{listing.name}</strong></h1>
         ...
         <button className='w-full hover:bg-slate-600 cursor-pointer active:scale-[0.95] rounded-md h-[40px] bg-slate-700 text-white'>Send Message</button></Link>
   
    </div> :  <button onClick={() => setContactLandlord(true)} className=' m-auto hover:bg-slate-600 w-[52%] cursor-pointer active:scale-[0.95] rounded-md h-[40px] bg-slate-700 text-white'>Contact Landlord</button>
    }
   </div>
  )
}

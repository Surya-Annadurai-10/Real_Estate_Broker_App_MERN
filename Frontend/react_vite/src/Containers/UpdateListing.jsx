import {
  ...
  const UpdateListing = () => {
    ...
    useEffect(()=>{
        if(formData){
            console.log(formData.imageURLs, " update resData");
            setImageURLs([
                ...formData.imageURLs
            ])
        }
    ...
    const handleCreateListing = async () => {
      ...
      try {
        ...
        setDeleteStatus("");
        setCreating(true);
        const res = await fetch(`/api/listing/update-listing/${formData._id}`, {
          method: "POST",
          headers: {
       ...
       if(resData.success == true){
        ...
        navigate(`/profile/${resData.data.userRef}`)
       }
      } catch (error) {
    ...
    const handleDeleteImage = async (file, i) => {
      ...
      setUploadingStatus("");
      setDeleteStatus("");
       console.log(file , i, "filename to delete");
    ...
    return (
      <main className="bg-[#F1F5F1] h-[90vh] w-full">
        <div className="w-[95%] mx-auto  h-full">
          <h1 className="font-bold text-center text-2xl py-5">Update Listing</h1>
                ...
                <div className="flex items-center justify-start gap-3  ">
                  <div className="flex gap-1 ">
                    <input
                      ...
                      name="type"
                      id="sell"
                      checked = {formData.type == "sale" ? true : false}
                      type="radio"
                      onChange={(e) => handleChange(e)}
                    />
